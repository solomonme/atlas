...
func (oc *OrganizationCreate) SetRevenues(f float64) *OrganizationCreate {
	oc.mutation.SetRevenues(f)
	return oc
}

// SetSecurityScore sets the "security_score" field.
func (oc *OrganizationCreate) SetSecurityScore(f float64) *OrganizationCreate {
	oc.mutation.SetSecurityScore(f)
	return oc
}

// SetNillableSecurityScore sets the "security_score" field if the given value is not nil.
func (oc *OrganizationCreate) SetNillableSecurityScore(f *float64) *OrganizationCreate {
	if f != nil {
		oc.SetSecurityScore(*f)
	}
	return oc
}
...
func (oc *OrganizationCreate) AddAuditIDs(ids ...int) *OrganizationCreate {
	oc.mutation.AddAuditIDs(ids...)
	return oc
}

// AddAudits adds the "audits" edges to the Audit entity.
func (oc *OrganizationCreate) AddAudits(a ...*Audit) *OrganizationCreate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return oc.AddAuditIDs(ids...)
}
...
func (oc *OrganizationCreate) defaults() {
	if _, ok := oc.mutation.SecurityScore(); !ok {
		v := organization.DefaultSecurityScore
		oc.mutation.SetSecurityScore(v)
	}
}
...
func (oc *OrganizationCreate) check() error {
	...
	if _, ok := oc.mutation.Revenues(); !ok {
		return &ValidationError{Name: "revenues", err: errors.New(`ent: missing required field "Organization.revenues"`)}
	}
	if v, ok := oc.mutation.Revenues(); ok {
	...
	if _, ok := oc.mutation.SecurityScore(); !ok {
		return &ValidationError{Name: "security_score", err: errors.New(`ent: missing required field "Organization.security_score"`)}
	}
	if v, ok := oc.mutation.SecurityScore(); ok {
...
func (oc *OrganizationCreate) createSpec() (*Organization, *sqlgraph.CreateSpec) {
	...
	if value, ok := oc.mutation.Revenues(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeFloat64,
			Value:  value,
			Column: organization.FieldRevenues,
		})
		_node.Revenues = value
	}
	if value, ok := oc.mutation.SecurityScore(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeFloat64,
			Value:  value,
	...
	if nodes := oc.mutation.AuditsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
