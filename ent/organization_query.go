...
func (oq *OrganizationQuery) QueryAudits() *AuditQuery {
	query := &AuditQuery{config: oq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrganizationQuery) Clone() *OrganizationQuery {
	...
	return &OrganizationQuery{
		...
		withAudits: oq.withAudits.Clone(),
...
func (oq *OrganizationQuery) WithAudits(opts ...func(*AuditQuery)) *OrganizationQuery {
	query := &AuditQuery{config: oq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withAudits; query != nil {
		if err := oq.loadAudits(ctx, query, nodes,
			func(n *Organization) { n.Edges.Audits = []*Audit{} },
			func(n *Organization, e *Audit) { n.Edges.Audits = append(n.Edges.Audits, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrganizationQuery) loadAudits(ctx context.Context, query *AuditQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Audit)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Audit(func(s *sql.Selector) {
		s.Where(sql.InValues(organization.AuditsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
