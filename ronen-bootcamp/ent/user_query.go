...
func (uq *UserQuery) QueryAudits() *AuditQuery {
	query := &AuditQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAudits:        uq.withAudits.Clone(),
...
func (uq *UserQuery) WithAudits(opts ...func(*AuditQuery)) *UserQuery {
	query := &AuditQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAudits; query != nil {
		if err := uq.loadAudits(ctx, query, nodes,
...
func (uq *UserQuery) loadAudits(ctx context.Context, query *AuditQuery, nodes []*User, init func(*User), assign func(*User, *Audit)) error {
	...
	query.Where(predicate.Audit(func(s *sql.Selector) {
		s.Where(sql.InValues(user.AuditsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_audits
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_audits" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_audits" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
